...
if [ "$TARGET" = "interactive" ]; then
    ...
    [ -f build/vm_runtime_arrstr.prg ]     && cp build/vm_runtime_arrstr.prg     "$FS/gpc.rt.arrstr.bin"
    [ -f build/vm_runtime_arrstrdata.prg ] && cp build/vm_runtime_arrstrdata.prg "$FS/gpc.rt.arrstrdata.bin"
    rm -f "$FS/out.prg"
